...
const handleToggle = (target) => {
  ...
  // Cek apakah sedang running di laptop sendiri atau di server
  const isLocal = hostName === 'localhost' || hostName === '127.0.0.1';

  // JIKA LOKAL: Tembak langsung ke 8000 (Daphne Laptop)
  // JIKA VPS: Tembak ke IP/Domain tanpa port (Biar Nginx yang arahkan)
  ...
  const handleClickOutside = (event) => {
    // Menutup menu jika klik terjadi di luar container menu 3 titik
...
const handleImageZoom = (e, imagePath, isLiveMode = false) => {
  ...
  if (isLiveMode) {
    ...
    // MODE HISTORY/AI: Muncul di sebelah KIRI
...
const handleReScreening = async (r) => {
  ...
  try {
    // 1. Pastikan BASE_URL benar-benar bersih dari spasi/karakter aneh
    const cleanBase = BASE_URL.trim();
    const cleanPath = API_URL.trim().replace(/^\/+|\/+$/g, "");
    
    // 2. Gunakan constructor URL untuk validasi otomatis
    // Ini akan membuang otomatis jika ada slash liar di depan http
    const urlObj = new URL(`${cleanPath}/${r.id}/re_screening/`, cleanBase);
