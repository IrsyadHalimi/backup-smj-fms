...
import MainTableGridFuelRecap from "./components/layout/MainTableGridFuelRecap";
  ...
  const fetchAllData = async (targetDate = '', search = '') => {
    ...
    try {
      ...
      const processedData = extractedData
        ...
        .map(r => {
          const kalkulasiSelisih = (r.hm_km_unit || 0) - (r.last_hm || 0); 
          const kalkulasiLtrJam = kalkulasiSelisih > 0 
          ? (r.consumed_qty || 0) / kalkulasiSelisih 
          : 0;

          return {
            ...r, // Menyalin semua field asli
            hm_selisih: kalkulasiSelisih, // Menambahkan field baru hasil kalkulasi
            ltr_jam: kalkulasiLtrJam.toFixed(2)
          };
        });

      // 🕵️ LOG DEBUGGING BAWAAN DIPERTAHANKAN (Menggunakan processedData.length agar akurat setelah difilter)
      console.log(`Berhasil Mengeset Pool Data: ${processedData.length} baris masuk ke state.`);

      // Set state menggunakan data yang sudah diproses dan dimanipulasi
      setDbHistory(processedData);
