...
function App() {
  ...
  // Konfigurasi API
  const BASE_URL = 'http://103.214.112.59:8000';
  const WS_URL = 'ws://103.214.112.59:8000/ws/fuel-sync/';
  const API_PATH = '/api/fuel-transactions/';
  ...
  const fetchSingleRecord = async (id) => {
    try {
      const response = await fetch(`${BASE_URL}${API_PATH}${id}/`);
...
const handleImageZoom = (e, imagePath, isLiveMode = false) => {
  ...
  const FuelTable = ({ data, title, isLive }) => (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      <div style={{ padding: '10px 14px', backgroundColor: activeTheme.tableHead, color: activeTheme.tableTitle, fontSize: '13px', fontWeight: '700', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${activeTheme.border}`, letterSpacing: '0.3px' }}>
        <span>{title}</span>
        {isLive && isNewDataIn && <span style={{ color: '#10b981', fontSize: '11px', fontWeight: '700', animation: 'blink 1s infinite' }}>● LIVE TRANSACTION</span>}
      </div>

      <div className="custom-scroll" style={{ overflowX: 'auto', overflowY: 'auto', flex: 1, minWidth: 0, backgroundColor: isDarkMode ? 'rgba(0,0,0,0.12)' : '#ffffff' }}>
  ...
  <tr style={{ fontSize: '11px', color: activeTheme.subText }}>
    ...
    {isLive ? (
      <>
        {/* Header Grup untuk Live Monitor */}
      ...
      <>
        {/* Header Grup untuk AI Screening */}
  ...
  {/* BARIS KEDUA: Sub-Header (Detail Kolom) */}
  <tr style={{ fontSize: '11px', color: isDarkMode ? '#cbd5e1' : '#1e293b' }}>
    {isLive ? (
      /* Sub-header Live: Tanpa AI & OCR Status, urutan: Device, FV, Foto */
      ...
      /* Sub-header AI Screening: Lengkap dengan AI & OCR Status */
  ...
  {(() => {
    // 1. Definisikan Tanggal Hari Ini (Jam 00:00) untuk perbandingan akurat
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    // 2. Filter Data berdasarkan kondisi Tabel
    const filteredData = data.filter(r => {
      // Ambil status dan parsing tanggal record
      const status = r.screening_status || 'Pending';
      const recordDate = new Date(r.date);
      recordDate.setHours(0, 0, 0, 0);

      if (isLive) {
        // LOGIKA LIVE: Sembunyikan jika sudah 'Downloaded' dan tanggal sudah lewat
        if (status === 'Downloaded' && recordDate < today) {
          return false;
        }
      } else {
        // LOGIKA AI SCREENING: Sembunyikan jika sudah 'Verified' dan tanggal sudah lewat
        // Kita biarkan status lain (Mismatch/Error) tetap muncul meskipun tanggal lama agar di-follow up
        if (status === 'Verified' && recordDate < today) {
          return false;
        }
      }
      return true;
    ...
    return filteredData.map((r, i) => {
      const statusGabungan = r.screening_status || 'Pending';
      let vColor = "#94a3b8";
      if (statusGabungan === 'Verified') vColor = "#10b981";
      else if (statusGabungan === 'Downloaded') vColor = "#38bdf8"; // Warna Biru khusus Downloaded
      else if (statusGabungan === 'Mismatch') vColor = "#ef4444";
      else if (statusGabungan === 'Processing') vColor = "#06b6d4";

      const imgStyle = {
        height: '36px', width: '52px', objectFit: 'cover', borderRadius: '4px',
        cursor: 'zoom-in', border: '1px solid rgba(255,255,255,0.1)',
        transition: 'transform 0.2s ease-in-out', animation: 'fadeInUp 0.4s ease-out'
      };
      ...
      return (
        <tr key={`${r.id}-${i}`} className={isLive && i === 0 && isNewDataIn ? "row-highlight" : ""} style={{ borderBottom: `1px solid ${activeTheme.border}` }}>
          ...
          {isLive ? (
            ...
            <>
              ...
              <td style={{ padding: '6px', borderRight: `1px solid ${activeTheme.border}` }}>
                {r.entry_type === 'INITIAL' ? <div style={{ width: '40px', height: '40px' }} /> : (
                  <img src={getImageUrl(r.hm_foto_full)} onClick={(e) => handleImageZoom(e, r.hm_foto_full, true)} onMouseLeave={() => setZoomImage(null)} style={imgStyle} alt="HM" />
                )}
              </td>
              ...
              <td style={{ padding: '6px', borderRight: `1px solid ${activeTheme.border}` }}>
                <img src={getImageUrl(r.flow_meter_foto_full)} onClick={(e) => handleImageZoom(e, r.flow_meter_foto_full, true)} onMouseLeave={() => setZoomImage(null)} style={imgStyle} alt="Flow" />
              </td>
              ...
              <td style={{ padding: '10px' }}>
                <div style={{ 
                  fontSize: '11px', fontWeight: 'bold', padding: '4px 12px', borderRadius: '20px', textAlign: 'center',
                  backgroundColor: r.screening_status === 'Error AI' ? 'rgba(239, 68, 68, 0.15)' : 'rgba(255,255,255,0.05)',
                  color: r.screening_status === 'Error AI' ? '#ef4444' : vColor,
                  border: `1px solid ${r.screening_status === 'Error AI' ? '#ef4444' : 'transparent'}`
                }}>
                  {statusGabungan}
                </div>
              </td>
            ...
            /* ========================= BODY HISTORY/AI MODE ========================= */
            <>
              <td style={{ padding: '6px' }}>
                {r.entry_type === 'INITIAL' ? <div style={{ width: '40px', height: '40px' }} /> : (
                  <img src={getImageUrl(r.hm_foto_full)} onClick={(e) => handleImageZoom(e, r.hm_foto_full, false)} onMouseLeave={() => setZoomImage(null)} style={imgStyle} alt="HM" />
                )}
              </td>
              <td style={{ padding: '10px', fontWeight: 'bold' }}>{r.hm_km_unit || 0}</td>
              ...
              <td style={{ padding: '10px', borderRight: `1px solid ${activeTheme.border}` }}>
                {(() => {
                  ...
                  if (!r.ai_hm_read) return <OCRStatusLabel status="Error AI" />;
                  return <OCRStatusLabel status={Number(r.hm_km_unit) === Number(r.ai_hm_read) ? 'Matched' : 'Mismatch'} />;
                })()}
              </td>
              <td style={{ padding: '6px' }}>
                <img src={getImageUrl(r.flow_meter_foto_full)} onClick={(e) => handleImageZoom(e, r.flow_meter_foto_full)} onMouseLeave={() => setZoomImage(null)} style={imgStyle} alt="Flow" />
              </td>
              <td style={{ padding: '10px', fontWeight: 'bold' }}>{r.flow_meter_value || 0}</td>
              ...
              <td style={{ padding: '10px', borderRight: `1px solid ${activeTheme.border}` }}>
                {(() => {
                  if (r.entry_type === 'INITIAL') return "-";
                  if (r.screening_status === 'Processing' || r.screening_status === 'Queue') return <OCRStatusLabel status="Processing" />;
                  if (r.ai_flow_read === null || r.ai_flow_read === undefined || r.ai_flow_read === "") return <OCRStatusLabel status="Error AI" />;
              ...
              <td style={{ position: 'relative', padding: '10px' }} className="slide-menu-container">
                ...
                <div style={{
                  ...
                  transition: 'all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)', boxShadow: '0 4px 15px rgba(0,0,0,0.3)', whiteSpace: 'nowrap'
                }}>
                  <button className="menu-item-slide" onClick={() => handleOpenValidation(r)} style={menuButtonStyle}>
                    <i className="fas fa-check-double" style={{color: '#10b981'}}></i> <span>Validation</span>
                  </button>
                  <div style={{ width: '1px', backgroundColor: 'rgba(255,255,255,0.2)', height: '15px', alignSelf: 'center' }} />
                  <button className="menu-item-slide" onClick={() => handleReScreening(r)} style={menuButtonStyle}>
                    <i className="fas fa-robot" style={{color: '#c084fc'}}></i> <span>Re-Screening</span>
                  </button>
                </div>
              </td>
            </>
...
const handleSubmitValidation = async () => {
  ...
  try {
    const response = await fetch(`${BASE_URL}${API_PATH}${selectedRecord.id}/validate/`, {
      method: 'PATCH',
...
const handleReScreening = async (r) => {
  ...
  try {
    const response = await fetch(`${BASE_URL}${API_PATH}${r.id}/re_screening/`, {
      method: 'POST',
    });
  ...
  {isReadyToExport ? 'EXPORT SPREADSHEET' : 'VALIDATION INCOMPLETE'}
      ...
      <div style={{ display: 'flex', flex: 1, gap: '10px', minHeight: 0, position: 'relative' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '12px', minHeight: 0, position: 'relative' }}>
          ...
          <div style={{ flex: 1, backgroundColor: activeTheme.card, borderRadius: '8px', border: `1px solid ${activeTheme.border}`, overflow: 'hidden' }}>
            <FuelTable data={filteredLive} title="LIVE TRANSACTION MONITOR" isLive={true} />
          </div>
          <div style={{ flex: 1.5, backgroundColor: activeTheme.card, borderRadius: '8px', border: `1px solid ${activeTheme.border}`, overflow: 'hidden' }}>
            <FuelTable data={filteredHistory} title="AI SCREENING MONITOR" isLive={false} />
          </div>
        </div>
