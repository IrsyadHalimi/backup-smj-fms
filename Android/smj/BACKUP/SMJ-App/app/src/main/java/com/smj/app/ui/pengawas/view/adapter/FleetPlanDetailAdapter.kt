...
class FleetPlanDetailAdapter(
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        if (s == "start" && mData[position].getStatus() != "delete"){
                ...
                if(isNetworkAvailable(mContext)) {
                    if (different.toInt() == 0) {
                        if (s == "start") {
                            ...
                            alertBuilder.setMessage(HtmlCompat.fromHtml("Mohon melakukan pencatatan <strong>Loading Time</strong> terlebih dahulu!", HtmlCompat.FROM_HTML_MODE_LEGACY))
                        ...
                        tvCircleTime.isEnabled = false
                        val alertBuilder = AlertDialog.Builder(mContext)
                        alertBuilder.setTitle("Mohon Menunggu")
...
//                    tvLocked.visibility = View.GONE
    ...
    fun setUnlocked(
        ...
        databaseReference = refRitase.child("Ritase").child(taskId.toString()).child(unitId.toString()).child(
            ritaseId
        )
        databaseReference.updateChildren(ritaseHashMap)
            .addOnCompleteListener { task ->
    ...
    fun setUndo(
        ...
        databaseReference = refRitase
            .child("Ritase")
            .child(taskId.toString())
            .child(unitId.toString()).child(ritaseId)
        databaseReference.updateChildren(ritaseHashMap)
            .addOnCompleteListener {
                val refShareContact = FirebaseDatabase.getInstance().reference
                ritase = ArrayList()
                        ...
                        override fun onDataChange(snapshot3: DataSnapshot) {
                            ...
                            if(snapshot3.exists()) {
                                ...
                                for (postSnapshot3 in snapshot3.children) {
                                    ...
                                    if (ritaseDataList3?.getStatus() != "canceled" && ritaseDataList.getSupirId() == supirId && ritaseDataList.getUnitId() == unitId) {
                                        c += 1
                                        tvCount.text = c.toString()
