...
class UnitAdapter(
        ...
        init {
            detail.setOnClickListener {
                ...
                if (mData[bindingAdapterPosition].getStatus() != "RFU" && forUse?.isNotEmpty() == true) {
                    checkUserCurrent(bindingAdapterPosition, unitType)
                }
                else {
    ...
    interface UnitAdapterCallback {
        fun onDetail(adapterPosition: Int, product: ArrayList<UnitList>)

    }

    private fun checkUserCurrent(bindingAdapterPosition: Int, unitType: TextView) {
