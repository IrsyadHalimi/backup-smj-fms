...
class ProductListActivity : AppCompatActivity(), UnitAdapter.UnitAdapterCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val exaCode = intent.getStringExtra("exaCode")
        ...
        refContact.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if(snapshot.exists()){
                    ...
                    for (dataSnapshot in snapshot.children) {
                        ...
                        if (unitType == "DUMP TRUCK" || unitType == "RIGID TRUCK") {
                            if (product?.getUnitType().equals(unitType)
                                ...
                                || product?.getUnitType().equals("DT")
                                || product?.getUnitType().equals("ADT")
                                || product?.getUnitType().equals("OHT")
                                || product?.getUnitType().equals("HD")
                                )
                            {
                    ...
                    val productAdapter = UnitAdapter(this@ProductListActivity, productList!!, this@ProductListActivity, forUse, unitType, binding.llProgressBar, taskId, exaCode)
                    ...
                    for (dataSnapshot in snapshot.children) {
                        ...
                        if (unitType == "DUMP TRUCK" || unitType == "RIGID TRUCK") {
                            if (unit?.getUnitType().equals(unitType)
                                ...
                                || unit?.getUnitType().equals("DT")
                                || unit?.getUnitType().equals("ADT")
                                || unit?.getUnitType().equals("OHT")
                                || unit?.getUnitType().equals("HD"))
                            {
    ...
    override fun onDetail(
        ...
        intent.putExtra("loc", dataTask?.getLocationName())
        intent.putExtra("locode", dataTask?.getLocationCode())
        intent.putExtra("galianName", dataTask?.getGalianName())
        intent.putExtra("galianCode", dataTask?.getGalianCode())
        intent.putExtra("timbunanName", dataTask?.getTimbunanName())
        intent.putExtra("timbunaCode", dataTask?.getTimbunanCode())
        intent.putExtra("jarak", dataTask?.getJarak())
        intent.putExtra("sopir", user?.getFullName())
        intent.putExtra("sopirId", user?.getUid())
        intent.putExtra("userPosition", user?.getPosition())
