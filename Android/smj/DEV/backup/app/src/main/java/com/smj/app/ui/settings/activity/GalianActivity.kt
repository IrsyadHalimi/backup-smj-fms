...
import com.smj.app.databinding.ActivityGalianBinding
...
class GalianActivity : AppCompatActivity(), LocationAdapter.LocationAdapterCallback {
    private lateinit var binding: ActivityGalianBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityGalianBinding.inflate(layoutInflater)
        ...
        val refContact = FirebaseDatabase.getInstance().reference.child("Galian")
        refContact.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if(snapshot.exists()){
                    ...
                    recyclerViewLocation.layoutManager = LinearLayoutManager(this@GalianActivity)
    ...
    private fun addingLocation(
        ...
        if (location != ""){
            ...
            databaseReference = refLocation.child("Galian").child(locationId)
            databaseReference.updateChildren(locationHashMap)
                .addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        Helper().showToast("Adding data is Successful!", this@GalianActivity)
                        binding.llProgressBar.preload.visibility = View.GONE
                    }
                    else{
                        binding.llProgressBar.preload.visibility = View.GONE
                        Helper().showToast("Data Product is Something Wrong!!!", this@GalianActivity)
                    }
                }
        }
    }
    ...
    private fun updateLocation(
        ...
        if (location != ""){
            ...
            databaseReference = refShift.child("Galian").child(locationId)
            databaseReference.updateChildren(shiftHashMap)
                .addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        Helper().showToast("Update data is Successful!", this@GalianActivity)
                        binding.llProgressBar.preload.visibility = View.GONE
                    }
                    else{
                        binding.llProgressBar.preload.visibility = View.GONE
                        Helper().showToast("Data Location is Something Wrong!!!", this@GalianActivity)
                    }
                }
        }
    }
                ...
                override fun onDataChange(p0: DataSnapshot) {
                    if (p0.exists()) {
                        ...
                        if (user?.getPosition() == "ADMIN" || user?.getPosition() == "root") {
                            ...
                            val alertBuilder = android.app.AlertDialog.Builder(this@GalianActivity)
