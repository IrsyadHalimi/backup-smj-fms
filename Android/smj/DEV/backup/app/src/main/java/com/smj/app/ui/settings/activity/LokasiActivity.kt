...
import com.smj.app.databinding.ActivityLokasiBinding
...
class LokasiActivity : AppCompatActivity(), LocationAdapter.LocationAdapterCallback  {
            ...
            .addValueEventListener(object : ValueEventListener {
                ...
                override fun onDataChange(p0: DataSnapshot) {
                    if (p0.exists()) {
                        ...
                        if (user?.getPosition() == "Manager HRGA" || user?.getPosition() == "MT MCC" || user?.getPosition() == "MCC Field" || user?.getPosition() == "Jr.Foreman MCC") {
        ...
        refContact.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if(snapshot.exists()){
                    ...
                    recyclerViewLocation.layoutManager = LinearLayoutManager(this@LokasiActivity)
    ...
    private fun addingLocation(
        ...
        if (location != ""){
            ...
            databaseReference = refLocation.child("Locations").child(locationId)
            databaseReference.updateChildren(locationHashMap)
                .addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        Helper().showToast("Adding data is Successful!", this@LokasiActivity)
                        binding.llProgressBar.preload.visibility = View.GONE
                    }
                    else{
                        binding.llProgressBar.preload.visibility = View.GONE
                        Helper().showToast("Data Product is Something Wrong!!!", this@LokasiActivity)
                    }
                }
        }
    }
    ...
    private fun updateLocation(
        ...
        if (location != ""){
            ...
            databaseReference = refShift.child("Locations").child(locationId)
            databaseReference.updateChildren(shiftHashMap)
                .addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        Helper().showToast("Update data is Successful!", this@LokasiActivity)
                        binding.llProgressBar.preload.visibility = View.GONE
                    }
                    else{
                        binding.llProgressBar.preload.visibility = View.GONE
                        Helper().showToast("Data Location is Something Wrong!!!", this@LokasiActivity)
                    }
                }
        }
    }
                ...
                override fun onDataChange(p0: DataSnapshot) {
                    if (p0.exists()) {
                        ...
                        if (user?.getPosition() == "Manager HRGA" || user?.getPosition() == "MT MCC" || user?.getPosition() == "MCC Field" || user?.getPosition() == "Jr.Foreman MCC") {
                            ...
                            val alertBuilder = android.app.AlertDialog.Builder(this@LokasiActivity)
